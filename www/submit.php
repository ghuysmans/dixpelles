...
if (empty($_SESSION['user']))
	...
	if (empty($_POST['orig']) || empty($_POST['orig_text']) || empty($_POST['target']) || empty($_POST['target_text']) || empty($_POST['url']))
		...
		$q = $db->prepare('INSERT INTO mistranslation(orig, orig_text, target, target_text, url, submitted_by) VALUES (?, ?, ?, ?, ?, ?)');
		if ($q->execute(array($_POST['orig'], $_POST['orig_text'], $_POST['target'], $_POST['target_text'], $_POST['url'], $_SESSION['user']))) {
